...
/**
 * Ultimate Mortgage Calculator — One-Page (No Tabs) Recode
 * --------------------------------------------------------
 * This version implements the user's requests on Sep 8, 2025:
 * • Remove the default text header "Ultimate Mortgage Calculator"
 * • Add the Ultimate Mortgage Calculator logo to the TOP‑LEFT of the site header
 * • When Borrower View is selected, show a footer INSIDE the RIGHT COLUMN:
 *     - bottom‑left: Extreme Loans logo + "Extreme Loans © 2025" and under it "NMLS: #2025962"
 *     - bottom‑right: "Prepared September 8, 2025"
 *
 * Notes:
 * - Put the logo file in /public/mortgage-calc-logo.png (already provided in this chat).
 * - If you also keep /Extreme-Loans-Logo-Color.png and /Extreme-Loans-Logo-White.webp in /public,
 *   the footer logo will render (non‑borrower view).
 */
...
export default function MortgageCalculatorMismo() {
  // ===== Theme =====
  const [accentColor, setAccentColor] = useState<string>("#4f46e5"); // indigo-600
  ...
  const remainingBalance = (principal: number, ratePct: number, totalYears: number, monthsPaid: number) => {
    ...
    if (r === 0) return principal - pmt * k; // linear paydown
    ...
    <div className={lightMode ? "" : "dark"}>
      <div className={`min-h-dvh ${lightMode ? "bg-white" : "bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950"}`}>
        ...
        <div className="mx-auto max-w-7xl px-4 py-4 flex items-center justify-between gap-4" style={{ color: lightMode ? undefined : "#e5e7eb" }}>
          {/* LEFT: Replace text header with the Ultimate Mortgage Calculator logo */}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              <span className={textSecondary}>Theme Color</span>
            </button>
            <input
            ...
            {armType !== "None" && (
              <div className="space-y-3">
                ...
                <p className={`text-sm ${textSecondary}`}>
                  ...
                  This is a <strong>2‑step plan\n</strong>: enjoy the lower start rate now, then refinance before the adjustment period in {armYears} years.
                </p>
                <div className="grid grid-cols-2 gap-3">
...
{/* ===== Right Column Footer (ternary-free & bullet-proof) ===== */}
<div className="mt-6 pt-5 border-t border-slate-700/40">
  {/* Borrower View footer (visible only when borrowerView is true) */}
  <div className={borrowerView ? "grid grid-cols-3 items-end" : "hidden"}>
    ...
    {/* CENTER: Fixed logo block with NMLS directly under it */}
  ...
  {/* Non-borrower view footer (visible only when borrowerView is false) */}
